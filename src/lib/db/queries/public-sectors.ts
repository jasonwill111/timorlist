/**
 * Public Sectors Query Functions
 * 统一 public sectors 列表查询
 */
...
export async function getPublicSectorCategories(): Promise<{ parentCats: CategoryRecord[]; categoryMap: Record<string, string>; childMap: Record<string, string[]> }> {
  const db = await getRawDb();
  if (!db) return { parentCats: [], categoryMap: {}, childMap: {} };

  try {
    const allCats = (await db.prepare(
      'SELECT id, name, parent_id as parentId FROM public_sector_categories'
...
export async function getPublicSectors(
  ...
  if (childCat) {
    ...
    const childMap = (await getPublicSectorCategories()).childMap;
  ...
  try {
    const countResult = await db.prepare(
      `SELECT COUNT(*) as count FROM public_sectors ${whereClause}`
    ...
    const dataResult = await db.prepare(`
      ...
      FROM public_sectors ${whereClause}
      ORDER BY created_at DESC LIMIT ? OFFSET ?
    `).bind(...params, limit, offset).all();

    const results = (dataResult as unknown as { results: PublicSector[] } | null)?.results || [];
